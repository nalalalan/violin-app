...
The present implementation of \texttt{violin.aolabs.io} uses an accepted-source gate: each image must be a full back view of a fine, antique, historically important, or high-end violin from a museum, public archive, or fine-instrument publication. The current source file excludes violin fronts, scrolls, labels, generic annotated diagrams, unrelated instrument views, partial crops, and source-detail plates. It includes 41 accepted full-back sources: 23 Metropolitan Museum of Art backs, 5 Library of Congress full-back views, and 13 fine-instrument or open public-media full-back sources. Each accepted source appears as one rendered full-back tile.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.22\textwidth}p{0.18\textwidth}p{0.48\textwidth}}
        ...
        Rendered wall & 41 tiles & One accepted full-back source produces one rendered tile, preserving links to the source object, publication, or exact image source. \\
        \bottomrule
    \end{tabular}
\end{table}
...
Several limitations remain. The present corpus is curated rather than exhaustive. It privileges sources with usable high-quality back imagery and therefore reflects the availability of museum, archive, publication, and open-media photographs. Some living makers and fine modern instruments are underrepresented because public back images are less consistently available or less clearly licensed. The site also has not measured user outcomes. The strongest next study would compare practice initiation, session duration, affect before practice, and voluntary return rates across a back-only wall, a general violin gallery, and a non-visual practice dashboard.
...
Museum sources were drawn from public collection records, including The Metropolitan Museum of Art. Public-archive sources were drawn from Library of Congress object records, including Stradivari, Guarneri del Gesu, and Amati instruments. Additional sources came from fine-instrument publication pages and open public-media pages when the image itself was a full back. Source links are preserved in the rendered wall so that visual attention can move into provenance inspection when desired.

\subsection*{Design rationale}

The wall uses dense masonry-style rendering rather than a small slideshow or educational lesson. This choice supports rapid comparison among backs, keeps the page visually rich, and reduces the number of explicit choices required before looking. Captions remain hidden until hover or focus so that the first perceptual object is the instrument surface rather than explanatory text. The paper link is placed in a top strip to make the manuscript accessible without interrupting the image wall.
