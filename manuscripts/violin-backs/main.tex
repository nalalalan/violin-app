...
The present implementation of \texttt{violin.aolabs.io} uses an accepted-source gate: each image must be a full back view of a fine, antique, historically important, or high-end violin from a museum, public archive, notable-sale archive, fine-instrument dealer, fine-instrument shop, or open public-media source. The current source file excludes violin fronts, scrolls, labels, generic annotated diagrams, unrelated instrument views, partial crops, and source-detail plates. It includes 756 accepted full-back sources: 23 Metropolitan Museum of Art backs, 5 Library of Congress full-back views, 13 fine-instrument or open public-media full-back sources, 567 Ingles \& Hayday notable-sale full-back views, 135 Corilon fine/dealer full-back views, and 13 SHAR fine-instrument full-back views. Each accepted source appears as one rendered full-back tile.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/fig01_violin_back_wall.png}
    \caption{Source-gated violin-back media wall. The deployed wall isolates full violin backs rather than mixing backs with fronts, scrolls, labels, diagrams, detail crops, or generic violin images. The source set is deliberately narrow: fine, antique, historically important, or high-end instruments only.}
    \label{fig:wall}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.22\textwidth}p{0.18\textwidth}p{0.48\textwidth}}
        ...
        Rendered wall & 756 tiles & One accepted full-back source produces one rendered tile, preserving links to the source object, publication, product page, or exact image source. \\
        \bottomrule
    \end{tabular}
\end{table}
...
The paper and site update were based on the current \texttt{violin.aolabs.io} source in \texttt{public/app.js}, inspected on 12 May 2026. The source defines a \texttt{backSources} corpus and maps each accepted source to exactly one rendered tile. Source entries are accepted only when the image is a full violin back and the instrument is fine, antique, historically important, or otherwise high-end. The current implementation contains 756 accepted full-back sources and 756 rendered tiles.

\subsection*{Inclusion and exclusion rules}

Accepted media include only full violin backs when the source context identifies the instrument as fine or historically meaningful. Excluded media include fronts, scrolls, labels, bridges, f-holes, generic diagrams, annotated plates, low-quality auction thumbnails, partial crops, corner details, waist details, lower-back-only details, back-plate construction diagrams, uncertain identity images, and images whose source context does not support inclusion. The exclusion rule is intentionally strict because the scientific object of the page is the complete back surface, not violin imagery in general.

\subsection*{Source categories}

Museum sources were drawn from public collection records, including The Metropolitan Museum of Art. Public-archive sources were drawn from Library of Congress object records, including Stradivari, Guarneri del Gesu, and Amati instruments. Additional sources came from Ingles \& Hayday notable-sale pages, Corilon fine-instrument pages, SHAR fine-instrument product pages, fine-instrument publication pages, and open public-media pages when the image itself was a full back. Source links are preserved in the rendered wall so that visual attention can move into provenance inspection when desired.

\subsection*{Design rationale}

The wall uses dense masonry-style rendering rather than a small slideshow or educational lesson. This choice supports rapid comparison among backs, keeps the page visually rich, and reduces the number of explicit choices required before looking. The deployed wall uses three large columns at desktop and wide desktop widths and one column on mobile, so full backs remain legible rather than collapsing into small thumbnails. Captions remain hidden until hover or focus so that the first perceptual object is the instrument surface rather than explanatory text. The paper link is placed in a top strip to make the manuscript accessible without interrupting the image wall.
