...
\vspace{0.35em}
\noindent\textbf{A violin is usually introduced through its front, label, maker, sound, price, or performer, yet the back is the surface where craft history, material selection, varnish, wear, and ownership converge into one continuous visual object. Here we present \texttt{violin.aolabs.io}, a source-gated media wall that deliberately isolates full fine-violin backs from fronts, scroll-only views, labels, annotations, partial crops, detail plates, and generic instrument imagery. The current source file contains a broad 763-record full-back corpus, while the deployed wall renders 159 stricter display-gated sources selected for pale studio presentation, one-piece or visually continuous flame, and explicit inclusion of historically important makers such as Stradivari and Amati. We argue that this back-only design is more than an aesthetic preference. It creates a low-friction visual environment for musical attention: the player can enter the instrument through beauty, material literacy, historical continuity, and craft desire before practice begins. Drawing on evidence from arts-and-health research, self-determination theory, neuroaesthetics, aesthetic-appreciation models, and music-practice research, we define a testable design hypothesis: dense exposure to high-quality violin-back imagery may support mood regulation, identity, and practice re-entry by increasing autonomous interest and reducing the affective cost of starting. The claim is environmental rather than clinical; no user-outcome experiment is reported here. The contribution is a source-backed design framework for treating the violin back as a serious cultural, motivational, and attentional surface.}
...
The present implementation of \texttt{violin.aolabs.io} uses two gates. The broad source gate admits only full back views of fine, antique, historically important, or high-end violins from museum, public-archive, notable-sale, fine-instrument dealer, fine-instrument shop, and open public-media sources. The display gate is stricter: the rendered wall favours pale studio images, one-piece or visually continuous maple flame, and named historical importance, while excluding mismatched side views, dark-background photographs, weak thumbnails, detail crops, fronts, labels, and scroll-only media. The inspected source file contains 763 broad full-back records: 23 Metropolitan Museum of Art backs, 5 Library of Congress full-back views, 13 fine-instrument or open public-media full-back sources, 567 Ingles \& Hayday notable-sale full-back views, 135 Corilon fine/dealer full-back views, 13 SHAR fine-instrument full-back views, and 7 Reuning \& Son high-value full-back views. The deployed wall currently renders 159 display-gated tiles.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/fig01_violin_back_wall.png}
    \caption{Source-gated violin-back media wall. The deployed wall isolates full violin backs rather than mixing backs with fronts, scroll-only views, labels, diagrams, detail crops, or generic violin images. The current display gate favours pale studio images, one-piece or continuous flame, and historically important makers while preserving source links for provenance inspection.}
    \label{fig:wall}
\end{figure}
...
\begin{table}[H]
    \centering
    \caption{Current source gate for \texttt{violin.aolabs.io}. The table reports the source categories inspected in the application code on 12 May 2026.}
    ...
    \begin{tabular}{p{0.22\textwidth}p{0.18\textwidth}p{0.48\textwidth}}
        ...
        Rendered wall & 159 tiles & A stricter display gate selects full pale-background backs with one-piece or visually continuous flame, plus explicit Stradivari, Amati, Guarneri, and other historically important full-back views. \\
        \bottomrule
    \end{tabular}
\end{table}
