...
The present implementation of \texttt{violin.aolabs.io} uses an accepted-source gate: each image must be a back view or back detail of a fine violin from a museum, public archive, or fine-instrument publication. The current source file excludes violin fronts, scrolls, labels, generic annotated diagrams, and unrelated instrument views. It includes 37 accepted sources: 11 Metropolitan Museum of Art backs, 20 Library of Congress back views or back details, and 6 direct fine-instrument or public-media back sources. These expand into 131 rendered wall tiles through repeated full-body and detail crops.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/fig01_violin_back_wall.png}
    \caption{Source-gated violin-back media wall. The deployed wall isolates back views and back details rather than mixing backs with fronts, scrolls, labels, diagrams, or generic violin images. The source set is deliberately narrow: fine, antique, historically important, or high-end instruments only.}
    \label{fig:wall}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.22\textwidth}p{0.18\textwidth}p{0.48\textwidth}}
        ...
        \midrule
        Museum collection & 11 & Full back images from The Metropolitan Museum of Art, including Stradivari, Amati, and other collection instruments~\cite{met_gould_1693}. \\
        Public archive & 20 & Library of Congress back views and back details from Stradivari, Guarneri del Gesu, and Amati instruments, including the 1704 ``Betts'' Stradivari~\cite{loc_betts_1704}. \\
        Fine/public media & 6 & Selected back or back-detail images from fine-instrument publication pages and open public media, including dedicated violin-back files~\cite{commons_violin_backs}. \\
        Rendered wall & 131 tiles & Accepted sources are expanded into full-body and detail variants while preserving links to the source object or publication. \\
        \bottomrule
    \end{tabular}
\end{table}
...
The paper and site update were based on the current \texttt{violin.aolabs.io} source in \texttt{public/app.js}, inspected on 12 May 2026. The source defines a \texttt{backSources} corpus and expands each source into rendered variants. Source entries are accepted only when the image is a violin back or back detail and the instrument is fine, antique, historically important, or otherwise high-end. The current implementation contains 37 accepted sources and 131 rendered tiles.

\subsection*{Inclusion and exclusion rules}

Accepted media include full violin backs, back details, maple-flame details, edgework details, lower-back views, waist details, and corner details when the source context identifies the instrument as fine or historically meaningful. Excluded media include fronts, scrolls, labels, bridges, f-holes, generic diagrams, annotated plates, low-quality auction thumbnails, uncertain identity images, and images whose source context does not support inclusion. The exclusion rule is intentionally strict because the scientific object of the page is the back surface, not violin imagery in general.

\subsection*{Source categories}

Museum sources were drawn from public collection records, including The Metropolitan Museum of Art. Public-archive sources were drawn from Library of Congress object records, including Stradivari, Guarneri del Gesu, and Amati instruments. Additional sources came from fine-instrument publication pages and open public-media pages when the image itself was a back or back detail. Source links are preserved in the rendered wall so that visual attention can move into provenance inspection when desired.
