...
\vspace{0.35em}
\noindent\textbf{Violin practice often begins before sound: with the player's willingness to approach the instrument again. Here we present \texttt{violin.aolabs.io}, a source-gated media wall that isolates full fine-violin backs from fronts, labels, scroll-only views, annotations, detail crops, and generic instrument imagery. The current source file contains 767 broad full-back records, while the deployed wall renders 163 stricter display-gated sources selected for pale studio presentation, visually continuous flame, historically important makers, and four user-selected instruments with acceptable public full-back images. We argue that the violin back is not a secondary surface but a compact record of craft history, material selection, varnish, wear, ownership, and desire. Drawing on arts-and-health research, self-determination theory, neuroaesthetics, aesthetic-appreciation models, and music-practice research, we frame the wall as an environmental intervention for attention and practice re-entry. No clinical or user-outcome claim is made; the contribution is a source-backed design framework for treating violin backs as serious cultural, motivational, and attentional media.}
...
The present implementation of \texttt{violin.aolabs.io} uses two gates. The broad source gate admits only full back views of fine, antique, historically important, or high-end violins from museum, public-archive, notable-sale, fine-instrument dealer, fine-instrument shop, user-selected source pages, and open public-media sources. The display gate is stricter: the rendered wall favours pale studio images, one-piece or visually continuous maple flame, named historical importance, and selected instruments whose available image is good enough to lead the wall, while excluding mismatched side views, dark-background photographs, weak thumbnails, detail crops, fronts, labels, and scroll-only media. The inspected source file contains 767 broad full-back records: 23 Metropolitan Museum of Art backs, 5 Library of Congress full-back views, 13 fine-instrument or open public-media full-back sources, 4 user-selected featured backs, 567 Ingles \& Hayday notable-sale full-back views, 135 Corilon fine/dealer full-back views, 13 SHAR fine-instrument full-back views, and 7 Reuning \& Son high-value full-back views. The deployed wall currently renders 163 display-gated tiles.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/fig01_violin_back_wall.png}
    \caption{Source-gated violin-back media wall. The deployed wall isolates full violin backs rather than mixing backs with fronts, scroll-only views, labels, diagrams, detail crops, or generic violin images. The current display gate restores the broader 163-tile wall while pinning four user-selected, source-backed full-back images ahead of the previous lead set.}
    \label{fig:wall}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.22\textwidth}p{0.18\textwidth}p{0.48\textwidth}}
        ...
        User-selected featured backs & 4 & The ``Ysaye'', ``Spagnoletti'', ex-Vieuxtemps, and Becker records were pinned ahead of the previous lead set after their source pages exposed usable full-back images~\cite{strings_ysaye_1740,nc_spagnoletti_1734,darnton_vieuxtemps_1710,shar_becker_1940}. \\
        ...
        Rendered wall & 163 tiles & A stricter display gate selects full pale-background backs with one-piece or visually continuous flame, explicit Stradivari, Amati, Guarneri, and other historically important full-back views, plus the four acceptable user-selected full-back additions. \\
        \bottomrule
    \end{tabular}
\end{table}
...
The paper and site update were based on the current \texttt{violin.aolabs.io} source in \texttt{public/app.js}, inspected on 25 May 2026. The source defines a broad \texttt{backSources} corpus, curated display-key arrays, a lead-key set, and an exclusion set. Source entries are admitted to the broad corpus only when the image is a full violin back and the instrument is fine, antique, historically important, otherwise high-end, or one of the selected named instruments with a usable full-back image. The current implementation contains 767 broad full-back records and 163 rendered display-gated tiles.

\subsection*{Inclusion and exclusion rules}

Accepted media include only full violin backs when the source context identifies the instrument as fine or historically meaningful. Excluded media include fronts, scroll-only images, labels, bridges, f-holes, generic diagrams, annotated plates, low-quality auction thumbnails, partial crops, corner details, waist details, lower-back-only details, back-plate construction diagrams, uncertain identity images, non-pale or visually noisy backgrounds, and images whose source context does not support inclusion. The display gate further removes otherwise valid backs that do not match the current aesthetic target: large readable full-back views with pale studio presentation, one-piece or visually continuous flame, explicit historical-maker importance, or a user-selected instrument whose public image is good enough to pin. The 1742 ``Lord Wilton'' Guarneri del Gesu remained outside the pinned set in this revision because the inspected public routes were low-resolution or composite rather than a strong standalone back image. The exclusion rule is intentionally strict because the scientific object of the page is the complete back surface, not violin imagery in general.

\subsection*{Source categories}

Museum sources were drawn from public collection records, including The Metropolitan Museum of Art. Public-archive sources were drawn from Library of Congress object records, including Stradivari, Guarneri del Gesu, and Amati instruments. Additional sources came from Ingles \& Hayday notable-sale pages, Corilon fine-instrument pages, SHAR fine-instrument product pages, Reuning \& Son high-value instrument pages, fine-instrument publication pages, and open public-media pages when the image itself was a full back. The added selected-instrument images came from Strings, NC Violins, Darnton \& Hersh, and SHAR public routes. Source links are preserved in the rendered wall so that visual attention can move into provenance inspection when desired.
